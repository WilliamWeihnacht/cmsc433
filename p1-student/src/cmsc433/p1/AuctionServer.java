...
public class AuctionServer
{
	...
	public int submitItem(String sellerName, String itemName, int lowestBiddingPrice, int biddingDurationMs)
	{
		...
		synchronized (instanceLock) {
			...
			itemsAndIDs.put(item.listingID(), item);
	...
	public List<Item> getItems()
	{
		...
		synchronized (instanceLock) {
			...
			List<Item> list = new ArrayList<Item>(itemsUpForBidding);
	...
	public boolean submitBid(int listingID, String bidderName, int biddingAmount)
	{
		...
		synchronized (instanceLock) {
			...
			if (item == null
					...
					|| !itemsUpForBidding.contains(item)	
					|| (highestBids.containsKey(listingID) && highestBids.get(listingID) >= biddingAmount)
					|| (!highestBids.containsKey(listingID) && biddingAmount < item.lowestBiddingPrice())
					|| !item.biddingOpen()
					|| blacklist.contains(bidderName)) {
				return false;
			}		
			
			//check if item has been bid on before
	...
	public int checkBidStatus(int listingID, String bidderName)
	{
		...
		synchronized (instanceLock) {
			...
			String high = highestBidders.get(listingID);
			...
			if (item != null) {
				if (item.biddingOpen()) {
					...
					if (high == null) {
						...
						if (itemsUpForBidding.remove(item)) {
					...
					} else if (high.equals(bidderName)) {
						...
						if (itemsUpForBidding.remove(item)) {
							...
							this.uncollectedRevenue += highestBids.get(listingID);
						}
				...
				if (highestBids.containsKey(listingID)) {
					...
					if (amount >= price) {
						...
						for (Item i: itemsUpForBidding) {
							if (highestBidders.get(i.listingID()).equals(bidderName)) {

								highestBids.remove(i.listingID());
								highestBidders.remove(i.listingID());
